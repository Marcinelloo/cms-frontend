...
export const findAllReviews = async (pageNumber) => {
  ...
  if (user) {
    return await API.get(
      `${GROUP_DEFAULT_URL}&filters[user][id][$ne]=${user.id}&sort=Title&pagination[page]=${pageNumber}&pagination[pageSize]=4`
    );
  } else {
    return await API.get(
      `${GROUP_DEFAULT_URL}&sort=Title&pagination[page]=${pageNumber}&pagination[pageSize]=4`
    );
  }
};
