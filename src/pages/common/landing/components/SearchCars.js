...
const SearchCars = () => {
  ...
  const searchCarsMutationByParameter = useMutation({
    mutationFn: (brand, model, fuel_type, year, mileage, minPrice, maxPrice) => findCarsByParameter(brand, model, fuel_type, year, mileage, minPrice, maxPrice),
    onSuccess: ({ data }) => {
      setData(data.data);
    },
  });
  ...
  const handleSearch = () => {
    ...
    const mileage = selectedMileage ? parseInt(selectedMileage.value, 10) : null;
    const minPrice = minPriceRef?.current?.value && parseInt(minPriceRef.current.value);
    const maxPrice = maxPriceRef?.current?.value && parseInt(maxPriceRef.current.value);

    if (selectedBrand || selectedBrand && selectedModel || selectedFuelType || selectedYear || mileage || minPrice || maxPrice) {
      searchCarsMutationByParameter.mutate({ brand, model, fuelType, year, mileage, minPrice, maxPrice });
    }
  };
    ...
    <>
      ...
      <Wrapper>
        <SelectWrapper>
          ...
          <SelectInput placeholder="przebieg"
